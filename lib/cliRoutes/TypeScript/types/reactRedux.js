...
module.exports =
  ...
  (function () {
    function TypesReactRedux () {
      _classCallCheck(this, TypesReactRedux)
    }

    _createClass(TypesReactRedux, [
      {
      ...
      },
      {
        ...
        value: function install (packageInstaller) {
          switch (packageInstaller) {
            case 'NPM':
              customCMD.get(
                ''.concat(typesReactRedux.install),
            ...
            case 'Yarn':
              customCMD.get(
                ''.concat(typesReactReduxYarn.install),
                function (err, data, stderr) {
                  err ? console.log(err) : errorLogging(stderr, data)
                },
                'install'
      ...
      },
      {
        ...
        value: function uninstall (packageInstaller) {
          switch (packageInstaller) {
            case 'NPM':
              customCMD.get(
                ''.concat(typesReactRedux.uninstall),
                function (err, data, stderr) {
                  err ? console.log(err) : errorLogging(stderr, data)
                },
                'uninstall'
            ...
            case 'Yarn':
              customCMD.get(
                ''.concat(typesReactReduxYarn.uninstall),
                function (err, data, stderr) {
                  err ? console.log(err) : errorLogging(stderr, data)
                },
                'uninstall'
