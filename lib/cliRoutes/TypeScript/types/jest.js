...
module.exports =
  ...
  (function () {
    function TypesJest () {
      _classCallCheck(this, TypesJest)
    }

    _createClass(TypesJest, [
      {
      ...
      },
      {
        ...
        value: function uninstall (packageInstaller) {
          switch (packageInstaller) {
            ...
            case 'Yarn':
              customCMD.get(
                ''.concat(typesJestYarn.uninstall),
                function (err, data, stderr) {
                  err ? console.log(err) : errorLogging(stderr, data)
                },
                'uninstall'
