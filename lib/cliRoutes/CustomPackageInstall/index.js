    ...
    value: function prompt() {
      _prompt(YarnOrNpm).then(function (_ref) {
        ...
        _prompt(multiplePackageInstall).then(function (_ref2) {
          ...
          if ((0, _toConsumableArray2["default"])(packages).includes('create-react-app')) {
            packages.shift();
            packages.join(' ');

            switch (packageManager) {
              case 'Yarn':
                customCMD.get("".concat(createReactAppYarn, " ").concat(folderName, " && cd ").concat(folderName, " && yarn add ").concat(packages.join(' '), " "), 'install', "Package(s): React (using npx create-react-app), and ".concat(packages.join(' '), " have been installed successfully!"));
                break;

              case 'NPM':
                customCMD.get("".concat(createReactApp, " ").concat(folderName, " && cd ").concat(folderName, " && npm install --save ").concat(packages.join(' ')), 'install', "Package(s): React (using yarn react-app), and ".concat(packages.join(' '), " have been installed successfully!"));
            }
          } else {
            switch (packageManager) {
              case 'Yarn':
                customCMD.get("yarn add ".concat(packages.join(' '), " "), 'install', "Package(s): ".concat(packages.join(' '), " have been installed successfully!"));
                break;

              case 'NPM':
                customCMD.get("npm install --save ".concat(packages.join(' ')), 'install', "Package(s): ".concat(packages.join(' '), " have been installed successfully!"));
            }
          }
        });
      });
