...
var ErrorLogging = require("../../customNodeCMD/customError.ts"); // Cli Model


var cliModel = require("../../cliModel.ts");

var multiplePackageInstall = cliModel.multiplePackageInstall;
var YarnOrNpm = cliModel.YarnOrNpm;

var _require = require("../../cliModel/install-commands.ts"),
        ...
        _prompt(multiplePackageInstall).then(function (_ref2) {
          ...
          if (_toConsumableArray(packages).includes("create-react-app")) {
            ...
            switch (packageManager) {
              case "Yarn":
                customCMD.get("".concat(createReactAppYarn, " ").concat(folderName, " && cd ").concat(folderName, " && yarn add ").concat(packages.join(" "), " "), function (err, data, stderr) {
                  err ? console.log(err) : ErrorLogging(stderr, data);
                }, "install");
                break;

              case "NPM":
                customCMD.get("".concat(createReactApp, " ").concat(folderName, " && cd ").concat(folderName, " && npm install --save ").concat(packages.join(" ")), function (err, data, stderr) {
                  err ? console.log(err) : ErrorLogging(stderr, data);
                }, "install");
            }
          } else {
            switch (packageManager) {
              case "Yarn":
                customCMD.get("yarn add ".concat(packages.join(" "), " "), function (err, data, stderr) {
                  err ? console.log(err) : ErrorLogging(stderr, data);
                }, "install");
                break;

              case "NPM":
                customCMD.get("npm install --save ".concat(packages.join(" ")), function (err, data, stderr) {
                  err ? console.log(err) : ErrorLogging(stderr, data);
                }, "install");
            }
          }
